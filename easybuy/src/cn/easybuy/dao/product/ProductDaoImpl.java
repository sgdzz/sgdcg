...
public class ProductDaoImpl extends BaseDao implements ProductDao {
	...
	public Integer update(Product product) {
		Integer count=0;
		try {
        	Object[] params = new Object[] {product.getName(),product.getFileName(),product.getPrice(),product.getStock(), product.getCategoryLevel1Id(),product.getCategoryLevel2Id(),product.getCategoryLevel3Id(),product.getId()};
        	String sql = " update easybuy_product set name=?,fileName=?,price=?,stock=?,categoryLevel1Id=?,categoryLevel3Id=?,categoryLevel3Id=? where id=? ";
			count=this.executeUpdate(sql, params);
        } catch (Exception e) {
            e.printStackTrace();
        }finally{
